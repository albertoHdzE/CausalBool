...
| **Suite reproducibility under concurrent load** | **2 SIGSEGV crashes in 5 full-suite runs**, on **two different tests** | Observed 2026-09-06. `TSK-ARCH-006` crashed at 07:36 with **exit 139**; `NANDTests.m` crashed the same way on a later run. Both happened while other jobs were running; every run with nothing competing has been clean (72/72). **My first reading of this was wrong and is withdrawn**: I attributed it to `TSK-ARCH-006` after a single observation, and the second crash landed on an unrelated test, so it is environmental and **not tied to any one test**. In both cases the test wrote its verdict and the kernel then died, so the runner scored FAIL on the non-zero kernel exit — correctly — and the pre-push hook refused the push. **Recorded OPEN, not explained.** It has a second consequence worth keeping: a crash leaves the previous artefact in place, so a stale verdict and a planting leftover cannot be told apart after the fact. |
