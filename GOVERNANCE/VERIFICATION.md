...
| **Files implementing an owned concept without loading the owner** | **2 real, out of 9 accused** — the other 7 are guard false positives. The matched/owner-referencing denominators this row used to quote are **WITHDRAWN as unmeasured** | `tools/check_core_loading.py` (AUDIT04 Phase C). **The detector was never negative-controlled, and it does not survive one.** Its `offset_subsetsum` Python rule fires on `range(n)` plus the bare words *free* / *subset* / *sum* occurring anywhere in code; its `gate_dispatch` rule is `"AND"` and `"OR"` and `"XOR"` present plus `def ` present. Measured 2026-09-06 on five snippets that cannot implement the concept, **3 of 5 were accused** — including a function named `probe_no_free_lunch` and a counter named `connectivity_subset`. Adjudicated by inspection, only `imp-pathinfo-paper/.../method_comparison.py` is a genuine mirror (both `description_length` and `repertoire`); it is the declared replication-fidelity exception. `imp-prices/tests/test_gate_network.py` is **the opposite of a mirror** — its hand-written expected truth tables are the pin, and forwarding it to the owner would have made it assert `owner === owner`. Planting proved the guard can catch a mirror; it never established that a match *means* one, and those are different claims. **Red by design until the detector is anchored and ships with the negative control.** |
