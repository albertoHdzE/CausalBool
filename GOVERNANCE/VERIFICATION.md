...
| Declared Python suite | **225** tests over **25** files (1 skipped, declared) | CI asserts the count; membership from `MANIFEST.tsv` |
| `src/` line+branch coverage | **29.12 %** over **61** files | `check_coverage_ratchet.py`, global + per-module floors |
| Complexity measures that are algorithmic | **3 / 3** — index-set, BDM, `D_v2` forwarder; 0 rank random below structured | `test_complexity_measures_are_algorithmic.py` |
...
| Test files classified | **120 / 120** — 85 Wolfram + 35 Python (97 test, 15 producer, 8 quarantine) | `check_test_manifest.sh` |
