...
| **Files implementing an owned concept without loading the owner** | **3 file-concept pairs across 2 files**, over **413 scanned / 74 matched / 53 referencing an owner / 19 excepted**. Detector specificity re-measured on every run: **8/8 negative, 4/4 positive** | `tools/check_core_loading.py` (AUDIT04 Phase C, anchored AUDIT04-D). **The first version was never negative-controlled and did not survive one.** Its `offset_subsetsum` rule fired on `range(n)` plus the bare words *free* / *subset* / *sum*; `gate_dispatch` on the literals `"AND"`, `"OR"`, `"XOR"` plus `def ` anywhere; and the Wolfram weights rule on `"ws"`, which matches the word **`rows`** — six occurrences were enough to accuse `CADetailLibrary.wl`. Of 9 accusations, **7 were false**: a probe named `no_free_lunch`, a counter named `connectivity_subset`, gate names inside a refusal assertion, `range(256)` over Boolean *functions*, and `imp-prices/tests/test_gate_network.py`, whose hand-written expected truth tables are **the pin** — forwarding it to the owner would have made it assert `owner === owner`. Every Python rule now demands **two or three independent structural signals** of the mechanism, and the control corpus runs **before the scan** on every invocation: the guard exits `2` and refuses to produce any count if it fails. Verified by removing one anchor — controls drop to 6/8, naming both false positives, and the guard refuses. **Anchoring also raised sensitivity**: it found `imp-causal-paper/.../boolean_network.py`, a genuine 2^n repertoire the word-matching rule had missed. The 3 remaining are real and await adjudication. |
