...
| **Coverage of `src/` as a whole** | **13 %** — 2,331 statements, 2,006 missed; 26 of 53 modules are imported by no test at all | The 98.56 % in §3 is real but covers **99 statements in two files**. This row exists so the scoped figure can never be read as the unscoped one. Raising it is a research-shaped job: much of `src/` is one-off analysis and scraping whose "correct" output is a scientific judgement, not an assertion. |
