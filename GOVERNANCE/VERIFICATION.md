...
| Replication packages | **28 / 97 / 47 / 42** | CI matrix, each count asserted |
...
| **Files implementing an owned concept without loading the owner** | **2 file-concept pairs in 1 file**, over **413 scanned / 75 matched / 53 referencing an owner / 21 excepted**. Detector specificity and sensitivity re-measured on every run: **8/8 negative, 5/5 positive** | `tools/check_core_loading.py` (Phase C, anchored AUDIT04-D). The first version matched bare English words — `range(n)` plus *free*/*subset*/*sum*; the literals `"AND"`/`"OR"`/`"XOR"` plus `def `; and `"ws"`, which matches the word **`rows`**. Of its 9 accusations **7 were false**, including `imp-prices/tests/test_gate_network.py`, whose hand-written truth tables are **the pin** — acting on that flag would have made the test assert `owner === owner`. Rules now demand two or three independent structural signals, and the control corpus runs **before the scan**: the guard exits `2` and emits no count at all if it fails. **Two ledger defects were found by the same pass.** `repertoire` declared only Wolfram owners, so no Python file could ever satisfy it — every Python 2^n enumeration was unfixable by construction; `index-deconvolution/src/causalbool.py` is the Python forward model (45/45 against `CausalBoolCore.wl`) and was simply never declared as one. And anchoring introduced a **false negative** of my own — the rule required uppercase gate literals, so a real dispatch on `"and"`/`"or"`/`"xor"` was never considered; the positive controls exist to catch exactly that, and did. `imp-pathinfo` is resolved: it reaches the owner via `load_root_modules`, and its `wiring_description_length` is the **index-set term alone** — measured **45 of 45 (n, degree, gate) cases disagree**, the gap being exactly `log2(12)` plus the gate's parameter payload, now pinned by a test that fails if either definition drifts. **The 2 remaining are one genuine mirror awaiting the author's decision:** `imp-causal-paper/.../boolean_network.py` holds a private `boolean_operator` and its own 2^n transition map, reaching no owner. |
| **Suite reproducibility under concurrent load** | `TSK-ARCH-006` is **5 clean of 6 runs**; the sixth took the kernel down with **exit 139 (SIGSEGV)** | Observed 2026-09-06: full suite OK at 07:19, **SIGSEGV at 07:36 while other jobs were running**, standalone ×3 OK, full suite OK at 07:46 with nothing competing. The test wrote `OK` and the kernel then crashed, so the runner scored it FAIL on the non-zero kernel exit — correctly, and the pre-push hook refused the push. Nothing in the test is heavy (`Tuples[{0, 1}, 6]` is 64 rows), so this reads as environmental. **Recorded OPEN, not explained**, and it has a second consequence: a crash leaves the previous artefact in place, so a stale verdict and a planting leftover are indistinguishable after the fact. |
