...
| Declared Python suite | **211** tests over **24** files | CI asserts the count; membership from `MANIFEST.tsv` |
| `src/` line+branch coverage | **29.90 %** over **61** files | `check_coverage_ratchet.py`, global + per-module floors |
...
| Test files classified | **119 / 119** — 85 Wolfram + 34 Python (96 test, 15 producer, 8 quarantine) | `check_test_manifest.sh` |
...
### The 8 quarantined Python files — quarantine never means passing

Made visible on 2026-09-07. Two of these are **red against real code** and are
...
|---|---|---|
| `Lev4/TSK-LEV4-INTEGRATION-001` | **3 failed, 2 passed** | `src/pipeline/Contingency_Monitor.py` returns `PUBLISH_EMERGENCE` where the test expects `PIVOT_CELL` once and `CONTINUE` twice. Either the decision policy moved without its test or the monitor is wrong — an adjudication for the author, so it is declared rather than silently retuned to whatever the code now prints. |
| `Nature/TSK-NATURE-SETUP-002` | **1 failed, 2 passed** | `HierarchyEncoder` `test_pure_dag` asserts a pure DAG costs **0** on its feedback term and measures **4.754887502163468 bits**. A non-zero feedback cost for an acyclic graph is either a real defect or a mis-stated invariant. |
