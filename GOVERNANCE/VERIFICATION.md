...
| Complexity measures that are algorithmic | **3 / 3** — **Variant A (row-run index-set)**, BDM, `D_v2` forwarder; 0 rank random below structured **on the chain family** | `test_complexity_measures_are_algorithmic.py` |
| Structured families ordered correctly | **BDM 5 / 5 · Variant A 3 / 5** — the index-set length inverts on a checkerboard (1050.5 bits against 563.9 for random of equal density) and on column stripes (1050.5 against 568.4), because it is a run-length code over rows and an alternating row costs its maximum. **Declared, not hidden**: `DECLARED_INVERSIONS` goes red if either disappears without the declaration moving | `test_structured_families_against_matched_random` |
...
| ~~**`D_v2` was a complexity measure**~~ **RETIRED 2026-09-07 — it was measuring complexity BACKWARDS** | random ranked simpler-or-equal than a chain: **`D_v2` 195/200 (97.5 %)** · BDM **0/200** · **Variant A (row-run index-set)** 14/200 (7.0 %) | `UniversalDv2Encoder._block_complexity` was Shannon binary entropy over block densities, and `_block_key` was `(size, ones)` — blocks keyed by density with the arrangement discarded. Binary entropy is **maximised at density one half**, the reverse of a program length, so every claim of the form "the real network is simpler than its randomised nulls" computed through it read the opposite of what it stated. It also gave a network and its transpose **identical** lengths (difference exactly 0.0), so "the hub drives everyone" and "everyone drives the hub" scored the same in a causality-first programme. Author directive: no Shannon quantity may be one of our measures; the two comparison measures are **Variant A (row-run index-set)** and **BDM**. `D_v2` now forwards to `row_run_index_set_length` — a forwarder, not a deletion, because 13 production files call it and one change site cannot drift. Over 210 corpus networks the median moves **106.32 → 334.43 bits**, all 210 changed, and the forwarder equals the owner exactly on every one. **No number in either active manuscript is affected**; `D_v2` appears in neither manuscript. The variant-letter call-out is the AUDIT04-H2.5 fix for the conflation of "the index-set program length" with the variant it names; Variant A is the one used in the two reported comparisons, Variant E (`D_schema`) is the declared primary mechanism-side measure and appears in neither, and the variant letter is now carried wherever the prose does not already disambiguate. |
...
| **Intermittent pre-push stall in the Wolfram tier** | cause **still unknown**; plant `zsh tools/test_h3_watchdog.sh`: **TIMEOUT at 60 s**, exit **142**, named `STALL-CAPTURE stage=test-plant ...`; licence-contention probe `zsh tools/run_h3_4_licence_contention.sh`: **30 / 30 PASS**, elapsed **5-6 s** (mean **5.40 s**, max **6 s**) under a concurrently held second kernel | The old failure was a **three-hour anecdote with no artefact**, and no macOS hang report exists for any `WolframKernel`; a silent block therefore needs the runner itself to bound and name it. `tools/run_closure.sh` now prints per-member timestamps, runs every member under a per-member watchdog, returns **exit 3** on timeout, and attempts `sample` / `lsof` capture when the PID is still live. The plant proves the timeout fires at the **production** pure-tier value, but because the watchdog `exec`s the child the planted PID is already gone by capture time, so the measured result is the honest one: **timing + PID only**, not a fake backtrace. The licence-contention hypothesis is **not reproduced** by 30 syntax runs under concurrent load; with **0 / 30** stalls, the correct statement is that this bounds the per-invocation stall probability below **9.5 % at 95 % confidence**, which does **not** exclude a rarer intermittent defect. What is guaranteed instead is that a future recurrence is a **named red**, never a silent hang. This closes with either a **live `sample` backtrace** from a future timeout or **30 clean `make ci-local` / pre-push hook invocations** under the new runner. |
...
| | before — retired `D_v2`, z-score | after — **Variant A (row-run index-set)** | after — BDM |
|---|---|---|---|
