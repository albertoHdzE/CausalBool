...
how systems respond to perturbations, how short rules can describe their
behaviour, and how such descriptions can support explanations and
predictions~\cite{hernandez2019aid,hernandez2021aid}. The present method is a
specialisation of that perspective to an exact, finite Boolean repertoire. It
should therefore be read as strongly aligned with the spirit of
Kolmogorov--Chaitin complexity, while remaining distinct from a computation
of Kolmogorov complexity itself.
...
inverse method starts with that repertoire and works one output column at a
time. For column $k$, a coordinate $i$ is functional exactly when there is a
state $x$ such that
\begin{equation}
  O_k[x]\ne O_k[x\mathbin{\oplus}2^i].
\end{equation}
The resulting set is the set of connected inputs that the behaviour itself
determines. Restricting the column to those inputs gives a reduced truth
table. Matching that table against the canonical gate family, or retaining
it as an explicit look-up table when no named gate matches, produces a
generative description that can be replayed independently.
...
repertoire.\\
Functional deconvolution & Detects connected inputs by paired single-bit
perturbations and reduces each output column to its functional truth table. &
Recovers functional dependence, not edges that the function never uses.\\
Gate identification & Matches a reduced truth table to canonical gates and
reports the full equivalence class of matching names. & Different internal
circuits or gate names can realise the same function.\\
...
Perturbation analysis & Supplies witnesses for whether changing one input can
change an output, giving deterministic functional-sensitivity and
counterfactual checks. & This is model-based intervention analysis, not a
general causal claim from observational data.\\
Prediction by replay & Evaluates a recovered function for a queried input and
checks the prediction by independent forward replay. & It is exact within the
model/data domain, not an out-of-sample forecast for noisy or incomplete data.\\
\bottomrule
\end{tabularx}
...
is the input to the inverse, paired perturbations expose functional inputs,
the reduced behaviour is named or retained exactly, and the schema view gives
a lossless expansion into output indices. The final panel closes the loop by
replaying the recovered description and answering a deterministic query.
...
majority example is followed from its complete repertoire through paired
single-bit perturbations, recovery of connected inputs, schema-based
...
$\{7,15,23,31\}$. The diagram is a visual summary of finite exact
operations, not a claim that the method computes minimal Kolmogorov
complexity or identifies hidden wiring uniquely.}
...
that produces $x$ and is uncomputable in general. A fixed index-set encoding
therefore supplies a computable description length, and hence an upper-bound
candidate relative to that encoding, rather than $K(x)$ itself. The method
does not establish minimality across all programming languages, coding
schemes or representations. Its algorithmic character comes from describing
an individual deterministic object by a generative rule and testing whether
that rule reproduces the object exactly.

\subsection{Prediction, explanation and scale}

Once the functional inputs and reduced truth table have been recovered, a
new input can be evaluated without consulting the original hidden circuit.
Schema membership gives the same answer for an input in the represented
domain, while a paired state $x\mathbin{\oplus}2^i$ supplies a direct witness
for whether coordinate $i$ can affect the output. In this limited but precise
sense, the method supports prediction and explanation together: the predicted
output is accompanied by the recovered rule, the inputs on which it depends,
and the exact repertoire-replay check. This is the model-driven explanatory
role that motivates the perturbation framework in the earlier work
\cite{hernandez2019aid,hernandez2021aid}.
