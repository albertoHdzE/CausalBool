...
\subsection{CausalBool-assisted arithmetic route}
The additive implementation provides an implementation-level connection
between the Boolean method and the arithmetic questions. Its Boolean DAG
evaluator calls the existing CausalBool gate semantics~\cite{causalbool}; a
separate lowering pass then emits the frozen serialized rows
$A(w)B(w)=C(w)$ used by the challenge evaluator and Circom exporter. The
evaluator is used to generate and replay gate values, while the serialized
rows remain the acceptance boundary. This keeps witness generation separate
...
Q5 & 193 & 321 & 64-bit range relation compiled through a Boolean DAG\\
Q6 & 1,376 & 2,499 & 254-bit canonical comparison and exclusion of one\\
Q7 & 200 & 407 & complete width-four multiplier and lower-bound checks\\
\bottomrule
\end{tabularx}
\end{center}

For Q5, the packed scalar is bound to 64 Boolean bits. For Q6, the scalar is
packed into 254 bits, constrained below $P$, and required to differ from one.
For Q7, the multiplier retains all eight product bits and its stage carries;
the public product is range-bound and the factors are required to be at least
two. The verifier checks every $16^3=4096$ width-four assignment against the
independent integer predicate: 16 satisfy the relation and 4,080 are rejected.
This is an exact finite implementation result. It does not extend the
CausalBool Q7 construction to 64 bits or provide a CausalBool Q8 circuit.
