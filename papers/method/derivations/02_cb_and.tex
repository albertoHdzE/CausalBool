...
\end{array}
\]
If node 1 uses AND with inputs from nodes 2 and 3 (\( I_c = \{2, 3\} \)), the output is 1 only at index 8, where \( v_2 = v_3 = 1 \). The structure hinges on connected nodes’ contributions, forming a baseline index (the decimal anchor \( P \), GOVERNANCE/GLOSSARY.md sec.1a), while disconnected nodes (\( I_{nc} = \{1\} \)) shift this baseline.
