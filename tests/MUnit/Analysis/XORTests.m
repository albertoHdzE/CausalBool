...
avgSensitivityXOR[d_Integer] := Module[{inputs, changeCount = 0},
  inputs = Table[IntegerDigits[x, 2, d], {x, 0, 2^d - 1}];
  Do[
    With[{y = Integration`Gates`ApplyGate["XOR", v]},
      Do[
        With[{yf = Integration`Gates`ApplyGate["XOR", ReplacePart[v, j -> 1 - v[[j]]]]},
          If[yf != y, changeCount++]
        ], {j, 1, d}]
    ], {v, inputs}];
  N[changeCount/(2^d)]
...
emp = Table[avgSensitivityXOR[d], {d, 1, 4}];
th = Table[d, {d, 1, 4}];
...
Export[FileNameJoin[{base, "Patterns.json"}], <|"XOR_XOR" -> pat|>, "JSON"];
