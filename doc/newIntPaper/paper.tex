...
Historically, information integration frameworks motivated sensitivity and structural measures but suffered from probabilistic ambiguities. Here we abandon integration and pursue fully deterministic causal reconstruction. The manuscript consolidates algorithms and experiments compiled in \texttt{expProcess.tex}, with ordering conventions, canonical programmes, and tests. A key contribution of this work is the application of these tools to biological systems, where we show that the "Mechanistic Description Length" ($D$) serves as a powerful predictor of functional importance, outperforming purely topological or behavioural metrics.
...
\subsection{Biological Application Methods}
\textbf{Data Acquisition:} We curate Gene Regulatory Networks (GRNs) from Cell Collective and GINsim, standardising them into a JSON format. The dataset includes Lambda Phage (4 nodes), E. coli Lac Operon (7 nodes), Fission Yeast Cell Cycle (8 nodes), and T-cell Activation (12 nodes).

\textbf{Mechanistic Description Length ($D$):} We define a functional $D(\mathcal{N}) = \sum_i (D_{gate}^{(i)} + D_{input}^{(i)} + D_{param}^{(i)})$ that measures the information content of the causal rules.

\textbf{Gene Essentiality Prediction ($\Delta D$):} We simulate single-gene knockouts (stuck-at-0) and compute the change in description length, $\Delta D = D_{original} - D_{knockout}$. Genes with high $\Delta D$ are hypothesised to be essential.

\textbf{Behavioral Complexity (BDM):} We use the Block Decomposition Method (BDM) on the networks' attractors and truth tables to estimate behavioral algorithmic probability, contrasting it with mechanistic $D$.
...
\subsection{Biological Validation Results}
We evaluated the framework on the curated GRN dataset to test the core hypotheses of algorithmic simplicity and causal structure.

\textbf{Algorithmic Simplicity:} Biological networks consistently exhibited lower mechanistic description lengths than randomised counterparts ($D_{bio} \ll D_{rand}$), confirming the hypothesis that biological evolution selects for algorithmic simplicity in causal rules. Furthermore, using a custom Micro-BDM metric for small string complexity, we confirmed that biological truth tables exhibit lower algorithmic information density ($\sim$1.6--2.5 bits/bit) compared to random strings ($\sim$2.76 bits/bit), extending the simplicity hypothesis to the behavioural domain. Standard compressors (e.g., Gzip) failed to compress these short strings, resulting in data expansion rather than compression, which necessitated and validated the use of the custom Micro-BDM approach.

\textbf{Gene Essentiality Prediction:} The mechanistic information loss metric ($\Delta D$) successfully predicted gene essentiality with a global accuracy of 83.9\% across the 31 genes analysed. Essential genes showed a mean algorithmic information loss approximately 1.3$\times$ higher than non-essential genes.

\textbf{Mechanism-Behaviour Decoupling:} We observed a weak negative correlation ($r \approx -0.31$) between mechanistic $\Delta D$ and behavioural $\Delta BDM$. Essential genes often exhibit high $\Delta D$ (mechanistic impact) but low $\Delta BDM$ (behavioural change), suggesting that essentiality is encoded in the causal structure rather than solely in the attractor landscape.

\textbf{Edge of Chaos Phase Transition:} Tuning the proportion of XOR gates ($p_{XOR}$) revealed a phase transition in algorithmic emergence. While mechanistic $D$ remained constant (~100 bits), behavioural $BDM$ exhibited a sharp increase (150 $\to$ $>6000$) at $p_{XOR} \approx 0.3$, identifying a critical "Edge of Chaos" regime where simple rules generate complex dynamics.
...
In the biological domain, the strong predictive performance of $\Delta D$ for gene essentiality validates the premise that information is physically encoded in the causal mechanism itself. The observed decoupling between mechanistic and behavioural complexity further challenges the assumption that essential functions must manifest as complex dynamics; rather, they may reside in the structural fragility of the causal rules.
