...
\begin{itemize}
    ...
    \item \textbf{Result (current coverage):} Pearson $r=-0.50$ ($p=0.39$), Spearman $\rho=-0.30$ ($p=0.62$), with $n=5$ DepMap-covered scaffold nodes in the current mapping.
    \item \textbf{Interpretation:} The direction of effect is consistent with the hypothesis that structural impact predicts essentiality, but the test is underpowered at present and cannot support a definitive claim.
\end{itemize}
...
\textbf{Essentiality Link (DepMap)} & N/A & Pearson $r=-0.50$ (n=5) & Directional support; currently underpowered. \\ \bottomrule
\end{tabular}%
}
\end{table}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Experiment 4.3: DepMap validation rerun (Pearson $r=-0.50$; $n=5$ nodes covered).
    \end{itemize}
