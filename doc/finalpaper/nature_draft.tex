...
\subsection{Exactness boundary and computed proxies (implementation)}
All $D$ quantities reported from repository pipelines are encoding-dependent proxies for mechanistic/structural description length under a frozen representation; no claim is made to compute Kolmogorov complexity exactly. Two proxy families are used and are not compared by magnitude across families:
\begin{itemize}
    \item \textbf{GRN corpus (universality):} $D_{\mathrm{gzip}}(cm)=\mathrm{len}(\mathrm{gzip}(cm.\mathrm{tobytes}()))$ on the adjacency matrix $cm$ (units: compressed bytes).
    \item \textbf{Cancer/DepMap analyses:} $D^{(v2)}(cm)$ computed by the universal $D^{(v2)}$ encoder on the adjacency matrix (units: bits).
\end{itemize}
We treat conclusions in terms of sign, rank, and within-proxy effect sizes under a fixed serialization and ordering policy.

\subsection{Knockout impact (Mechanistic Information Loss, $\Delta D$)}
For an in-silico knockout or node removal $i$, we define:
\begin{equation}
\Delta D_i = D(\mathcal{N}_i^{\mathrm{ko}}) - D(\mathcal{N})
\end{equation}
so $\Delta D_i>0$ indicates that the knockout increases description length (the node contributed to compressibility/efficiency). For structural node deletion on a graph $G$, the same convention reads $\Delta D(v)=D(G\setminus v)-D(G)$ under the context-appropriate $D$ proxy.
...
\begin{equation}
Z = \frac{\mu(D^{null}) - D(G_{bio})}{\sigma(D^{null})}
\end{equation}
A positive Z-score indicates statistically significant simplicity (biological networks are more compressible than their nulls).
