...
| `4ClaudeCode/claude-Nature/paper/paper3_algorithmic_corruption.tex` | Level 8 paper tree | side-branch manuscript | `hold` | medium | none before explicit Level 8 paper-branch policy | Active-looking paired-TCGA manuscript inside the live Level 8 workspace; not the main source-of-truth, but also not a safe archive target now. |
...
| `4ClaudeCode/claude-Nature/paper/results/` | Level 8 paper outputs | mixed paper-support output tree | `hold` | medium-high | none before an explicit paper-support output policy changes | A targeted subset of governance-support artifacts has now been restored from `3f9bd13`; they are provenance-bearing and should not be treated like disposable duplicated test outputs. |
...
| `results/tests/` vs historical `4ClaudeCode/claude-Nature/paper/results/tests/` | test outputs | root test-output tree versus previously duplicated paper-side test mirror | `hold` | medium | none unless a paper-packaging requirement overrides current redundancy evidence | Historical comparison shows the paper-side `tests/` tree was a strict subset of current `results/tests/` (114 overlapping files, 0 historical uniques), so keeping that mirror absent is now the cleaner policy. |
| `4ClaudeCode/claude-Nature/` -> common workspace folder | workspace consolidation | integrate Level 8 reproducibility workspace into a standard `workspaces/` layout | `verify-first` | high | complete path-abstraction stage before any move | Many scripts and provenance documents reference the literal `4ClaudeCode/claude-Nature/...` path; a naive move would break defaults and reduce reproducibility. |
