...
**100% of the 24,880 atoms** in the mirror corpus — 1,197 molecules, capped at the
first 200 molecules per dataset (scripts/causalbool_mirror.py `graphs[:200]`;
smaller datasets: ClinTox 199, FreeSolv 198) — with non-neighbour decoys
...
tests/            41 correctness checks (pytest --collect-only)
results/, figures/
```
...
.venv/bin/python -m pytest -q                    # 41 correctness checks
...
**Campaign status** — generated by `scripts/campaign_status.py` from the ledgers
(`results/campaign_status.txt`; regenerate with
`.venv/bin/python scripts/campaign_status.py`), quoted verbatim:
...
T-Hop is complete for all six families; Mix-Hop is complete; Graphormer is missing only
...
| recovers the generating mechanism | index-set | index set and gate for 100% of the 24,880 corpus atoms (200-molecules/dataset cap; see `results/causalbool_mirror.json`) |
