...
| Table 3 | 13 of 18 blocks | campaign 607 runs / 202 of 216 cases |
...
- Molecule as a Boolean network: `C` = bond adjacency, gate per atom from
  ...
  MAJORITY).
- **Mechanism recovery is exact**: index set *and* gate for 100% of ~25,000 atoms
...
- **39 tests pass**: `.venv/bin/python -m pytest -q`
- **Training campaign**: 607 runs, 202 of 216 cases. Resume with the three
  per-model commands in `README.md`; notebooks glob `results/runs*.jsonl`.
